...
function UpdatePage({district, title, order}) {
    
    return (
        <Layout >
            <div className="input-group mb-3 ">
            <form action={`/api/orders/${order.id}`} method="Post" className="form-container">
                <label className="input-group-text">Название</label>
                <input type="text" name="name" className="form-control" value={order.name} />
                ...
                <input type="text" name='discount' className="form-control" value={order.discount}/>
                <label className="input-group-text">Состав заказа</label>
                <textarea type="text" name='description' className="form-control" value={order.description} ></textarea>
